...
from .tr_modules import TRIrreps
...
def orbital_analysis(atom_orbitals, required_block_type, spinful, targets=None, element_pairs=None, no_parity=False, verbose=''):
        ...
        for target in targets:
            ...
            for N_M_str, block_indices in target.items():
                ...
                if l1 is None and l2 is None:
                    ...
                    irreps_new, irreps_new_x0, _ = irreps_from_l1l2(l1, l2, 1, spinful, no_parity=no_parity)
                    net_out_irreps += irreps_new
                    if spinful:
                        net_out_tr += len(irreps_new_x0) * [1] + (len(irreps_new) - len(irreps_new_x0)) * [-1]
                else:
    ...
    if spinful:
        ...
        net_out_tr = net_out_tr + [-tr for tr in net_out_tr]
        # net_out_irreps = TRIrreps(net_out_irreps, net_out_tr)
    ...
    if verbose and required_block_type != 's':
        with open(verbose, 'w') as v:
            ...
            print(TRIrreps(net_out_irreps, net_out_tr), file=v)
