...
class e3LayerNorm(nn.Module):
    def __init__(self, irreps_in, tr_in=None, eps=1e-5, affine=True, normalization='component', subtract_mean=True, divide_norm=False):
        ...
        self.tr_in = tr_in
        if self.tr_in is not None:
            assert len(self.irreps_in) == len(self.tr_in)
        self.eps = eps
        
        if affine:          
            ib, iw = 0, 0
            weight_slices, bias_slices = [], []
            for index_ir, (mul, ir) in enumerate(irreps_in):
                if ir.is_scalar():
                    if (self.tr_in is None) or (self.tr_in is not None and self.tr_in[index_ir] == 1): # bias can to 0e or 0eE
    ...
    def forward(self, x: torch.Tensor, batch: torch.Tensor = None):
        ...
        for index, (mul, ir) in enumerate(self.irreps_in):        
            ...
            if self.divide_norm or (ir.l == 0 and (self.tr_in is None or self.tr_in[index] == 1)): # do not divide norm for l>0 or time-reversal-odd irreps if divide_norm=False
            ...
            if self.bias is not None and ir.is_scalar():
                if (self.tr_in is None) or (self.tr_in is not None and self.tr_in[index] == 1):  # bias can to 0e or 0eE
